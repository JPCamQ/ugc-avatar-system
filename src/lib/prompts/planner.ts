import { AvatarIdentity, PostIdea } from "../types";

export function getPostIdeasPrompt(avatar: AvatarIdentity, customContext?: string): string {
...
Por lo tanto, debes ambientar, localizar y enfocar las 5 ideas de publicaciones obligatoriamente en este contexto, adaptándolas coherentemente al estilo de vida de ${avatar.name}.`;
  ...
  return `Eres un estratega de contenido y director creativo para un influencer de Inteligencia Artificial (Avatar UGC).
...
export function getPromptFlowPrompt(
...
${avatar.videoSettings}`
  ...
  const avatarFirstName = avatar.name.split(" ")[0];

  const flowCharacterReferenceGuideline = `
CRITICAL FLOW CHARACTER REFERENCE RULE:
- In the DYNAMIC SCENE section (including all SHOTs/PHOTOs), whenever you refer to the avatar by name (either "${avatar.name}" or "${avatarFirstName}"), you MUST prefix it with empty brackets "[ ]" immediately before the name.
- Example: write "A photo of [ ] ${avatarFirstName} standing..." instead of "A photo of ${avatarFirstName} standing...".
- Example: write "[ ] ${avatarFirstName} is sipping coffee..." instead of "${avatarFirstName} is sipping coffee...".
- This is mandatory to allow the user to easily insert the Flow character reference tag (like @milena) inside the brackets. Never write "${avatarFirstName}" or "${avatar.name}" without "[ ]" right in front of it.
`;

  const physicalConsistencyGuideline = `
CRITICAL PHYSICAL VISUAL CONSISTENCY:
- Carefully read the avatar's HIGH-FIDELITY CHARACTER DNA: Master. [${avatar.characterDna}].
...
`;
  ...
  return `Eres un ingeniero de prompts experto en la plataforma "Flow de Gemini".
...
${flowCharacterReferenceGuideline}
${productGuideline}
${MAKEUP_PROTOCOL}
...
export function getInstagramCaptionPrompt(avatar: AvatarIdentity, idea: PostIdea): string {
  return `Eres ${avatar.name}, una Avatar UGC e Influencer de Inteligencia Artificial.
