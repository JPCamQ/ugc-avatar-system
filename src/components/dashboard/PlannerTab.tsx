    ...
    >
      ...
      <div className="md:col-span-2 bg-white/70 backdrop-blur-md border border-white/60 rounded-3xl p-5 flex flex-col justify-between max-h-[620px] shadow-lg shadow-slate-100/50">
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <button 
                      ...
                      className="opacity-0 group-hover:opacity-100 px-1.5 py-0.5 rounded bg-red-50 hover:bg-red-100 text-[9px] font-bold text-red-500 transition-all cursor-pointer border border-red-200/40"
                    >
                      Borrar
                    </button>
                  </div>
        ...
        <p className="text-[9px] text-slate-400 mt-4 text-center border-t border-slate-100 pt-2 leading-relaxed">
          {currentAvatar.name} documenta su vida cosmopolita de alta costura, gimnasio y viajes de lujo, inspirando de forma bilingüe.
        </p>
      </div>
        ...
        {selectedIdea ? (
          <div className="bg-white/70 backdrop-blur-md border border-white/60 rounded-3xl p-6 flex flex-col justify-between flex-1 max-h-[620px] overflow-y-auto no-scrollbar shadow-lg shadow-slate-100/50">
            <div>
              ...
              <div className="mb-4">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1.5 font-semibold">Escena Visual & Vestuario Dinámico (Modificable)</label>
                <textarea
              ...
              <div className="mb-4 p-4 bg-slate-50/70 border border-slate-200/40 rounded-2xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-[8px] font-bold text-slate-400 uppercase mb-1 font-semibold">Nombre Comercial del Producto</label>
                    <input
                      type="text"
              ...
              {selectedIdea.type === "video" && (
                <div className="mb-4 p-4 bg-slate-50/70 border border-slate-200/40 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div>
                    <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wide block font-semibold">Idioma de Voz (Flow)</span>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-[10px] font-bold border transition-all cursor-pointer ${audioLanguage === "voiceover" ? "bg-rose-50 border-rose-200 text-rose-600 shadow-sm" : "bg-white border-slate-200/50 text-slate-600 hover:bg-slate-50"}`}
                    >
              ...
              {promptOutput && (
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-[10px] font-bold text-amber-600 uppercase tracking-wider font-semibold">
                  ...
                  {promptSteps.length > 0 ? (
                    <div className="space-y-3">
                      ...
                      <p className="text-[9px] text-slate-500 leading-normal mb-2 font-medium">
                        ...
                        {promptSteps.map((step, idx) => (
                          <div key={idx} className="bg-slate-900 border border-slate-800 rounded-xl p-3.5 relative group flex flex-col justify-between gap-2 shadow-inner">
                            <div className="flex justify-between items-center">
                              <span className="px-2 py-0.5 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[8px] font-black uppercase tracking-wider font-bold">
                                {step.label}
                              </span>
                              <button
                                type="button"
                            ...
                            <div className="text-[9.5px] text-slate-400 font-sans italic leading-relaxed bg-slate-950 p-2.5 rounded-lg border border-slate-800 select-all max-h-[80px] overflow-y-auto no-scrollbar font-normal">
                              {step.text}
                            </div>
                          </div>
                        ))}
                    ...
                    <textarea
                      ...
                      value={promptOutput}
                      rows={6}
                      className="w-full bg-slate-900 border border-slate-850 rounded-xl px-4 py-3 text-[10px] text-slate-200 font-mono focus:outline-none resize-none leading-relaxed no-scrollbar select-all"
                    />
                  )}
              ...
              {captionOutput && (
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider flex items-center gap-1.5 font-semibold">
                  ...
                  <textarea
                    ...
                    value={captionOutput}
                    rows={6}
                    className="w-full bg-slate-50 border border-slate-200/50 rounded-xl px-4 py-3 text-xs text-slate-700 leading-relaxed focus:outline-none resize-none no-scrollbar select-all font-normal"
                  />
                </div>
              )}
